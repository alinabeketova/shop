...
function sortTable(colNum, type, id) {
    let elen = document.getElementById(id)
    let tbody = elen.querySelector('tbody')
...
function update_goods() {
    let reesult_price = 0
    let tbodu = document.querySelector('.list')
    tbodu.innerHTML = ""
    ...
    if(goods.length) {
        ...
        for(let i=0; i<goods.length; i++) {
            tbodu.insertAdjacentHTML('beforeend', 
            `
            <tr class="align-middle">
                <td>${i+1}</td>
                <td class="name">${goods[i][1]}</td>
                <td class="plice">${goods[i][2]}</td>
                <td>${goods[i][3]}</td>
                <td><buuton class="good_delete btn-danger" data-delete="${goods[i][0]}">&#10006;</button></td>
                <td><buuton class="good_delete btn-primary" data-goods="${goods[i][0]}">&#10149;</button></td>
            </tr>
            `
            )
            if(goods[i][4]>0) {
                goods[i][6] = goods[i][4]*goods[i][2] - goods[i][4]*goods[i][2]*goods[i][5]*0.01
                reesult_price += goods[i][6]
                document.querySelector('.cart').insertAdjacentHTML('beforeend', 
                `
                <tr class="align-middle">
                    <td>${i+1}</td>
                    <td class="price_name">${goods[i][1]}</td>
                    <td class="plice_one">${goods[i][2]}</td>
    ...
    document.querySelector('.price_result').innerHTML = reesult_price +'&#8381;'
}
...
document.querySelector('.list').addEventListener('click', function(e) {
    ...
    Swal.fire({
        title: 'внимание',
        text: 'хотите удалить товар?',
        icon: 'warning',
        showCancelButton: true,
        confirmButtonColor: '#3085d6',
        cancelButtonColor: '#d33',
        confirmButtonText: 'да',
        cancelButtonText: 'отмана',
    }).then((result) => {
